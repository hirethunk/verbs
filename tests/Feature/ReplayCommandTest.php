...
it('can replay events', function () {
    ...
    expect(app(TracksState::class)->load($state1_id, ReplayCommandTestState::class)->count)
        ->toBe(2)
        ->and($GLOBALS['replay_test_counts'][$state1_id])
        ->toBe(2)
        ->and(app(TracksState::class)->load($state2_id, ReplayCommandTestState::class)->count)
        ->toBe(4)
...
it('uses the original event times when replaying', function () {
    ...
    expect(app(TracksState::class)->load($state_id, ReplayCommandTestWormholeState::class)->time->unix())
